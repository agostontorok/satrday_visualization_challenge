...
df2$Heading.of.passengers <- df2$NBR.OF.PASSENGERS # this going to be the colour of the markers
df2$Amount.of.passengers <- df2$NBR.OF.PASSENGERS  # this is going to be the size of the markers

df2[df2$FLIGH.DIRECTION == "Incoming","Heading.of.passengers"] <- df2[df2$FLIGH.DIRECTION == "Incoming","Heading.of.passengers"] *-1

df2 <- aggregate(. ~ DESTINATION, df2[ ,c(1,4:5)], sum)
df2 <- df2[df2$Amount.of.passengers > 0, ] 
...
gg <- ggmap(map)
gg <- gg + geom_point(data = df2, 
                 aes(x = lonCi, y = latCi, size = Amount.of.passengers, 
                     fill = Heading.of.passengers) , colour="black", pch=21) 
...
df2$scaled_Amount.of.passengers <- df2$Amount.of.passengers / 30000
df2$Difference <- df2$Heading.of.passengers

# marker styling
m <- list(
  colorbar = list(title = "Millions USD"),
  size = ~scaled_Amount.of.passengers, 
  opacity = 0.8, 
  symbol = 'circle'
)
